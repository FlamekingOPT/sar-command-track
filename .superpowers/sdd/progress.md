...
Task 2: complete (weighted k-means clustering replaces greedy growth in mergeBlocksToZones; all pre-existing regression tests re-verified passing; 7 new tests across 5 describe blocks total — determinism/effort-balancing/branching-shape-regression(Y-shape)/hard-road-respect (4, original commit 11f10a6) + connectedParts direct-unit-test coverage (3: 2 added in fix round 2 commit bd4aa8e, 1 more added in the final-review fix pass). connectedParts is now a public export — the only API-surface change on this branch. Non-contiguous cluster splitting ended up covered by that direct unit test on connectedParts, not by an emergent black-box scenario — two attempts at a black-box fixture both failed to actually exercise the logic; full story in the plan-workspace ledger. One pre-existing test's expectation was deliberately changed ([1,3]->[2,2] zone widths) as an accepted trade-off of switching to k-means for extreme (10x+) weight skew on a near-1-D chain.)
